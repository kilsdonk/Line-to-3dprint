# modified_mesh_generator_holes.py - Enhanced version with proper hole support
import trimesh
import numpy as np
...
class EnhancedMeshGenerator:
    """
    Enhanced version of MeshGenerator that supports wall loops, filament IDs, and complex polygons with holes
    """
    ...
    def extrude_path(points, height, z_offset, name, filament_id, wall_loops=2):
        """
        Extrude a 2D path into a 3D mesh with metadata for OrcaSlicer
        Enhanced to handle complex polygons with holes
        
        Args:
            points: List of 2D points forming the path
        ...
        try:
            bottom_faces = EnhancedMeshGenerator.triangulate_polygon(points)
            faces.extend(bottom_faces)
            print(f"Bottom face triangulated with {len(bottom_faces)} triangles")
        except Exception as e:
            print(f"Bottom face triangulation failed: {e}")
            return None
        
        # Create top face triangles (reversed winding)
        try:
            top_faces = EnhancedMeshGenerator.triangulate_polygon(points)
            # Offset indices for top face and reverse winding order
            top_faces_offset = [[f[2] + n, f[1] + n, f[0] + n] for f in top_faces]
            faces.extend(top_faces_offset)
            print(f"Top face triangulated with {len(top_faces)} triangles")
        except Exception as e:
            print(f"Top face triangulation failed: {e}")
            return None
        ...
        try:
            ...
            if not mesh.is_watertight:
                ...
                print(f"Mesh ({name}) successfully repaired. Is watertight: {mesh.is_watertight}")
        except Exception as e:
...
class MeshGenerator:
    ...
    def extrude_path(points, height, z_offset, name, filament_id):
        """
        Enhanced version of the original extrude_path method with wall_loops support
        ...
        # Call the enhanced static method
